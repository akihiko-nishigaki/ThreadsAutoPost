...
function uploadToCloudflareR2(fileId, userId) {
  ...
  var canonical_headers =
    ...
    'host:' + host + '\n' +
    'x-amz-content-sha256:' + contentSha256 + '\n' +
    'x-amz-date:' + amzDate;  // ← 末尾の改行を削除
  var signed_headers = 'content-type;host;x-amz-content-sha256;x-amz-date';
  var canonical_request =
    'PUT\n' +
    canonical_uri + '\n' +
    '\n' +                       // クエリ文字列：空
    canonical_headers + '\n\n' + // ← 改行 2 回で「ヘッダー部→サイン済みヘッダー部」の区切りを表現
    signed_headers + '\n' +
    contentSha256;
...
function cloudflareFileUpload(fileId)
{
  ...
  // ログ出力
  Logger.log('Cloudflare R2にアップロードしたファイルのURL:', fileUrl);

  return fileUrl;
}

/**
 * Cloudflareにファイルをアップロードし、URLを指定したセルに保存する
 * @param {string} fileId - Google DriveのファイルID
 * @param {number} row - 保存先の行番号
 * @param {number} column - 保存先の列番号
 * @param {string} sheetName - 保存先のシート名
 * @returns {string} アップロードされたファイルのURL
 */
function cloudflareFileUploadToCell(fileId, row, column, sheetName = SHEETS_NAME.WEB_POST) {
  try {
    Logger.log('Cloudflareアップロード開始（セル保存） - fileId:', fileId);
    Logger.log('保存先セル:', sheetName, '行:', row, '列:', column);

    // 元のcloudflareFileUploadを呼び出し
    const cloudflareUrl = cloudflareFileUpload(fileId);
    
    // ログ出力
    Logger.log("cloudflareUrl:" + cloudflareUrl);
    ...
    // URLをセルに保存
    sheet.getRange(row, column).setValue(cloudflareUrl);
    Logger.log('URLをセルに保存しました:', cloudflareUrl);

    return cloudflareUrl;
  } catch (error) {
    Logger.log('Cloudflareアップロード（セル保存）でエラー:', error);
    throw error;
  }
}
