...
function onOpen() {
  ...
  const commonMenu = ui.createMenu('共通')
    ...
    .addItem('Cloudfrareデータ定期削除', 'setCloudflareCleanupOldFilesTrigger');
  ...
  const instaMenu = ui.createMenu('Instagram設定')
    ...
    .addItem('定期更新設定', 'setInstaTokenRefreshTrigger');
...
/**
 * Cloudflareの過去データ削除のトリガーを設定する
 */
function setCloudflareCleanupOldFilesTrigger() {
  Logger.log(`setCloudflareCleanupOldFilesTrigger:Start`);
  setRefreshTrigger(CONFIG.CLOUD_FLARE_DELETE_TRIGGER_ID, CONFIG.TRIGGER_FUNCTION_NAME_CLOUD_FLARE_DELETE);
  Logger.log(`setCloudflareCleanupOldFilesTrigger:END`);
