...
function uploadFile(base64Data, fileName, mimeType) {
  try {
    ...
    // ─── ここから追加 ───
    // 現在日時を取得し、yyyyMMdd_HHmmss 形式のタイムスタンプを生成
    const now = new Date();
    // スクリプトのタイムゾーンを使う（必要に応じて 'Asia/Tokyo' に固定してもOK）
    const timeZone = Session.getScriptTimeZone();
    const timestamp = Utilities.formatDate(now, timeZone, 'yyyyMMdd_HHmmss');
    // MIMEタイプから拡張子を抽出（例: 'image/png' → 'png'）
    const ext = mimeType.substring(mimeType.indexOf('/') + 1);
    // 新しいファイル名を組み立て
    const newFileName = `${timestamp}.${ext}`;
    // ─── ここまで追加 ───
    
    // base64をBlobに変換（ファイル名にタイムスタンプ＋拡張子を使用）
    ...
    let retIsSuccess, retFileUrl;
    if (isImage) {
      const result = handleImageUpload(blob, folderId);
      ...
      retFileUrl   = file.getUrl();
      setAttachmentFileUrl(retFileUrl);
      retIsSuccess = true;
    }
    
    console.log('アップロード結果:', retIsSuccess, retFileUrl);
    return { success: retIsSuccess, url: retFileUrl };
